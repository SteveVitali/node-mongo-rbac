        ...
        if ('roles' in user && user['roles'].length) {
            ...
            async.eachSeries(user['roles'], function(roleId, next) {
                ...
                _.each(roles, function(role) {
                    if ('resources' in role && role['resources']) {
...
Rbac.prototype.addPermissions = function(roleName, permissions, callback) {
    ...
    .findOne({ role: roleName }, function(err, role) {
        ...
        async.eachSeries(permissions, function(permission, next) { 
            // A permission is structured as { '/path/to/resource' : ['get', put', etc.] }
            ...
            role.markModified('resources'); // Tell Mongoose that "Mixed" field has been changed
            role.save( function(err) {
                console.log('Successfully added permissions to role', role);
                next(err);
            });
        }, callback);
    });
    ...
    .findOne({ role: roleName }, function(err, role) {
        ...
        async.eachSeries(permissions, function(permission, next) { 
            ...
            if (resource in resources) {
                ...
                role.markModified('resources'); // Tell Mongoose that "Mixed" field has been changed
                role.save( function(err) {
                    console.log('Successfully revoked permissions from role', role);
                    next(err);
                });
            } else {
